...
def QLRun(program,samWS,resWS,resnormWS,erange,nbins,Fit,wfile,Loop,Verbose,Plot,Save):
	...
	if Save:
		...
		if elastic:
			ei_path = os.path.join(workdir, fname+'_Eisf.nxs')					# path name for nxs file
			SaveNexusProcessed(InputWorkspace=fname+'_Eisf', Filename=ei_path)
			if Verbose:
				logger.notice('Output fit file created : ' + ei_path)
	...
	for nl in range(1,4):
		...
		for m in range(0,ns):
			...
			for i in range(0,nl):
				...
				YData[i*3].append(fw[i])
				YData[i*3+1].append(it[i])
				YData[i*3+2].append(i0[0])
				EData[i*3].append(fw[nl+i])
				EData[i*3+1].append(it[nl+i])
				EData[i*3+2].append(i0[1])
		...
		for i in range(0,nl):
			...
			dataY = np.append(dataY, np.array(YData[i*3+1]))
			dataE = np.append(dataE, np.array(EData[i*3+1]))
			Vaxis.append('ampl.'+str(nl)+'.'+str(i+1))

			#append width
			dataX = np.append(dataX, np.array(Xout))
			dataY = np.append(dataY, np.array(YData[i*3]))
			dataE = np.append(dataE, np.array(EData[i*3]))
			Vaxis.append('width.'+str(nl)+'.'+str(i+1))

			#append elastic
			dataX = np.append(dataX, np.array(Xout))
			dataY = np.append(dataY, np.array(YData[i*3+2]))
			dataE = np.append(dataE, np.array(EData[i*3+2]))
			Vaxis.append('elast.'+str(nl)+'.'+str(i+1))
...
def eisf(inWS,sname):
	i11 = '_i11'
	ExtractSingleSpectrum(InputWorkspace=inWS, OutputWorkspace=i11, WorkspaceIndex=1)
	mi11 = mtd[i11]
	e11 = '_e11'
	ExtractSingleSpectrum(InputWorkspace=inWS, OutputWorkspace=e11, WorkspaceIndex=2)
	me11 = mtd[e11]
	eisf11 = me11/(me11 + mi11)
	outWS = sname+'_Eisf'
	dataX = np.array(eisf11.readX(0))
	dataY = np.array(eisf11.readY(0))
	dataE = np.array(eisf11.readE(0))
	CreateWorkspace(OutputWorkspace=outWS, DataX=dataX, DataY=dataY, DataE=dataE, Nspec=1,
		UnitX='MomentumTransfer', VerticalAxisUnit='Text', VerticalAxisValues='eisf', YUnitLabel='')
	DeleteWorkspace(i11)
	DeleteWorkspace(e11)
	DeleteWorkspace(eisf11)
