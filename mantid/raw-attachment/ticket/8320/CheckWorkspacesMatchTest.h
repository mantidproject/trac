...
void testSaveAndLoad()
  {
    SaveNexusProcessed alg_save;
    LoadNexus alg_load;
    CheckWorkspacesMatch checker;
    
    if ( !alg_save.isInitialized() ) alg_save.initialize();
    MatrixWorkspace_sptr goodWS = WorkspaceCreationHelper::create2DWorkspaceWithRectangularInstrument (1, 5,1);
    
    goodWS->mutableSample().setName("samp");
    goodWS->mutableSample().setGeometryFlag(3);

    // Now set it...
    // specify name of file to save workspace to
    alg_save.setProperty("InputWorkspace", goodWS);
    std::string outputFile = "SaveNexusProcessedTest_testExec.nxs";
    TS_ASSERT_THROWS_NOTHING(alg_save.setPropertyValue("Filename", outputFile));

    if( Poco::File(outputFile).exists() ) Poco::File(outputFile).remove();
    
    TS_ASSERT_THROWS_NOTHING(alg_save.execute());
    TS_ASSERT(alg_save.isExecuted());

    std::string loaded = "loadedWS"; 
    if ( !alg_load.isInitialized() ) alg_load.initialize();
    alg_load.setProperty("Filename",outputFile); 
    alg_load.setProperty("OutputWorkspace",loaded); 
    TS_ASSERT_THROWS_NOTHING(alg_load.execute());
    TS_ASSERT(alg_load.isExecuted());
    MatrixWorkspace_sptr output = AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>(loaded);

    if (! checker.isInitialized()) checker.initialize(); 
    checker.setProperty("Workspace1",goodWS);
    checker.setProperty("Workspace2",output); 

    TS_ASSERT(checker.execute()); 
    TS_ASSERT_EQUALS(checker.getPropertyValue("Result"),"Success!" );

    if( Poco::File(outputFile).exists() ) Poco::File(outputFile).remove();
    Mantid::API::AnalysisDataService::Instance().remove(output->getName());
    Mantid::API::AnalysisDataService::Instance().remove(goodWS->getName());
