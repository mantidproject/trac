...
def applyCorrections(inputWS, canWS, corr, Verbose=False):
    ...
    ConvertUnits(InputWorkspace=CorrectedWS, OutputWorkspace=CorrectedWS, Target='DeltaE',
        EMode='Indirect', EFixed=efixed)
    ConvertSpectrumAxis(InputWorkspace=CorrectedWS, OutputWorkspace=CorrectedWS+'_rqw', Target='ElasticQ',
        EMode='Indirect', EFixed=efixed)
...
def abscorFeeder(sample, container, geom, useCor, Verbose=False, ScaleOrNotToScale=False, factor=1, Save=False,
    ...
    efixed = getEfixed(sample)                # Get efixed
    if container != '':
    ...
    if useCor:
        ...
        if ( container == '' ):
            ...
            ConvertSpectrumAxis(InputWorkspace=sub_result, OutputWorkspace=sub_result+'_rqw', Target='ElasticQ',
                EMode='Indirect', EFixed=efixed)
