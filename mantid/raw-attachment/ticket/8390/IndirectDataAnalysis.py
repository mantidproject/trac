...
def createFurySeqResFun(ties, par, option):
    ...
    if npeak == '2':
        ...
        if ties:
            fun += ';ties=(f1.Intensity=1-f2.Intensity-f0.A0)'
    return fun
...
def getFurySeqResult(inputWS, outNm, option, Verbose):
    ...
    if npeak == '2':
        ...
        if type == 'SE':
            B2 = params.column(13)  #beta2 value
    nHist = mtd[inputWS].getNumberHistograms()
...
def furyfitSeq(inputWS, func, ftype, startx, endx, Save, Plot, Verbose=False): 
    ...
    CopyLogs(InputWorkspace=inputWS, OutputWorkspace=fitWS)
    AddSampleLog(Workspace=fitWS, LogName="start_x", LogType="Number", LogText=str(startx))
    AddSampleLog(Workspace=fitWS, LogName="end_x", LogType="Number", LogText=str(endx))
    AddSampleLog(Workspace=fitWS, LogName="option", LogType="String", LogText=option)
    CopyLogs(InputWorkspace=inputWS, OutputWorkspace=outNm+"_Result")
    AddSampleLog(Workspace=outNm+'_Result', LogName="start_x", LogType="Number", LogText=str(startx))
    AddSampleLog(Workspace=outNm+'_Result', LogName="end_x", LogType="Number", LogText=str(endx))
    AddSampleLog(Workspace=outNm+'_Result', LogName="option", LogType="String", LogText=option)
    if Save:
        fpath = os.path.join(workdir, fitWS+'.nxs')					# path name for nxs file
        ...
        SaveNexusProcessed(InputWorkspace=outNm+'_Result', Filename=fpath)
        if Verbose:
...
def furyfitMult(inputWS, function, ftype, startx, endx, Save, Plot, Verbose=False):
    ...
    AddSampleLog(Workspace=fitWS, LogName="start_x", LogType="Number", LogText=str(startx))
    AddSampleLog(Workspace=fitWS, LogName="end_x", LogType="Number", LogText=str(endx))
    AddSampleLog(Workspace=fitWS, LogName="function", LogType="String", LogText=ftype)
    AddSampleLog(Workspace=fitWS, LogName="link_q", LogType="String", LogText='True')
    CopyLogs(InputWorkspace=inputWS, OutputWorkspace=outNm+'_result')
    AddSampleLog(Workspace=outNm+'_result', LogName="start_x", LogType="Number", LogText=str(startx))
    AddSampleLog(Workspace=outNm+'_result', LogName="end_x", LogType="Number", LogText=str(endx))
    AddSampleLog(Workspace=outNm+'_result', LogName="function", LogType="String", LogText=ftype)
    AddSampleLog(Workspace=outNm+'_result', LogName="link_q", LogType="String", LogText='True')
    if Save:
