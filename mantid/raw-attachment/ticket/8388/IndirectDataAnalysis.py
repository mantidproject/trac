...
def elwin(inputFiles, eRange, log_type='sample', Normalise = False,
    ...
    for file in inputRuns:
        ...
        if (nr == 0):
            LoadNexus(Filename=file, OutputWorkspace='run1')
    ...
    label = unit[0]+' / '+unit[1]
    CopyLogs(InputWorkspace='run1', OutputWorkspace=elfWS)
    AddSampleLog(Workspace=elfWS, LogName="vert_axis", LogType="String", LogText=label)
    e1WS = ename+'_eq1'
    ...
    label = unit[0]+' / '+unit[1]
    CopyLogs(InputWorkspace='run1', OutputWorkspace=e1WS)
    AddSampleLog(Workspace=e1WS, LogName="vert_axis", LogType="String", LogText=label)
    e2WS = ename+'_eq2'
    ...
    CopyLogs(InputWorkspace='run1', OutputWorkspace=e2WS)
    AddSampleLog(Workspace=e2WS, LogName="vert_axis", LogType="String", LogText=label)
    ...
    if unit[0] == 'Temperature':
        ...
        AddSampleLog(Workspace=e1WS, LogName="temperature_normalise", LogType="String", LogText=str(Normalise))
        if Normalise:
            eltWS = ename+'_elt'
            nhist = mtd[elfWS].getNumberHistograms()
            for n in range(0,nhist):
                ExtractSingleSpectrum(InputWorkspace=elfWS, OutputWorkspace='__lowTemp', WorkspaceIndex=n)
                y = mtd['__lowTemp'].readY(0)
                scale = 1.0/y[0]
                Scale(InputWorkspace='__lowTemp', OutputWorkspace='__lowTemp', Factor=scale, Operation='Multiply')
                if n == 0:
                    RenameWorkspace(InputWorkspace='__lowTemp', OutputWorkspace=eltWS)
                else:
                    ConjoinWorkspaces(InputWorkspace1=eltWS, InputWorkspace2='__lowTemp')
# need to create a vertical axis here eg VerticalAxisUnit='MomentumTransfer', VerticalAxisValues=q1
            CopyLogs(InputWorkspace='run1', OutputWorkspace=eltWS)
            unitx = mtd[eltWS].getAxis(0).setUnit("Label")
            unitx.setLabel(unit[0], unit[1])
            addElwinLogs(elfWS, label, eRange, Range2)
    DeleteWorkspace('run1')
    ...
    if Save:
        ...
        if Normalise:
            elt_path = os.path.join(workdir, eltWS+'.nxs')					# path name for nxs file
            SaveNexusProcessed(InputWorkspace=eltWS, Filename=elt_path)
            if Verbose:
                logger.notice('Creating file : '+elt_path)
...
def msdfit(inputs, startX, endX, Save=False, Verbose=False, Plot=True): 
    ...
    AddSampleLog(Workspace=msdWS, LogName="start_x", LogType="String", LogText=str(startX))
    AddSampleLog(Workspace=msdWS, LogName="end_x", LogType="String", LogText=str(endX))
    if Plot:
